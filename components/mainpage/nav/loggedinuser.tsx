  ...
  return (
    <Menu
        ...
        item: "duration-300 text-black hover:text-white rounded-2xl font-bold !bg-transparent",
            ...
            <Menu.Target>
                <div className="hidden w-36 items-center lg:flex normal-case relative before:absolute before:bg-ma-pink before:w-full before:h-full before:opacity-20 before:left-0 before:top-0 before:right-0 before:bottom-0 before:blur-md hover:before:opacity-0 before:duration-300 before:rounded-3xl duration-300 hover:bg-white px-5 text-black cursor-pointer py-2 bg-ma-pink rounded-3xl">
                    <span className='whitespace-nowrap text-ellipsis w-24 overflow-hidden'>{user?.user_metadata.name} </span>&gt;
                </div>
            </Menu.Target>
            <Menu.Dropdown>
            ...
            <Link href={router.pathname.includes('profile') ? "/dashboard" : '/dashboard/profile'} className="p-3 m-1 hover:bg-black hover:text-white duration-300 cursor-pointer rounded-2xl text-black flex items-center cursor-normal">
            {router.pathname.includes('profile') ? "Dashboard" : "Your Profile"} 
            </Link>
